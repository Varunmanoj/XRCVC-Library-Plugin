...
- Each cart item returns stored `addedAt` date and time and `addedByName`/`addedByMembershipId` when available. Per-item last-update fields are absent. Present the actor and cart owner separately as **actor Full Name (Membership ID) for owner Full Name (Membership ID)** when attribution is requested. `null` means a legacy item has no recorded value; never infer the actor or substitute the parent cart time or current time. For a requested item order, use `get_admin_cart(sort_by=addedAt|resourceTitle|resourceType|isRequestable|requestabilityStatus|addedByName|addedByMembershipId, sort_order=asc|desc)` or the same query parameters on `/carts/admin/{membershipId}` Markdown. The default is newest added first and missing dates sort last. `list_admin_carts` sorts cart summaries separately by `updatedAt`, `membershipId`, `fullName`, or `itemCount`; it does not sort items within a cart. Keep each selected sort and `pageInfo.nextCursor` together on structured pages.
...
- Do not call `get_admin_membership_id`, `list_admin_membership_ids`, or a profile tool to rediscover the cart owner's name. Do not treat `addedByName` as the cart owner's name; an on-behalf addition may have a different actor.
