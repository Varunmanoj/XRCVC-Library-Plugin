...
- Each cart item returns stored `addedAt` and `updatedAt` dates and times and `addedByName`/`addedByMembershipId` and `updatedByName`/`updatedByMembershipId` when available. Present the actor and cart owner separately as **actor Full Name (Membership ID) for owner Full Name (Membership ID)** when attribution is requested. `null` means a legacy item has no recorded value; never infer the actor or substitute the parent cart time or current time. For a requested item order, use `get_admin_cart(sort_by=addedAt|updatedAt|resourceTitle|resourceType|isRequestable|requestabilityStatus|addedByName|addedByMembershipId|updatedByName|updatedByMembershipId, sort_order=asc|desc)` or the same query parameters on `/carts/admin/{membershipId}` Markdown. The default is newest added first and missing dates sort last. `list_admin_carts` sorts cart summaries separately by `updatedAt`, `membershipId`, `fullName`, or `itemCount`; it does not sort items within a cart. Keep each selected sort and `pageInfo.nextCursor` together on structured pages.
