...
- Each cart item returns stored `addedAt` and `updatedAt` dates and times when available; `null` means the corresponding item time was not recorded. Do not substitute the parent cart time or current time. Member responses include the stored `addedByName` for every item, including self-added items. They exclude actor UIDs, actor Membership IDs, and last-updater identity; do not infer missing values. For a requested item order, use `get_member_cart(sort_by=addedAt|updatedAt|addedByName|resourceTitle|resourceType|isRequestable|requestabilityStatus, sort_order=asc|desc)` or the same query parameters on `/carts/member` Markdown. The default is newest added first, and missing dates sort last. Keep the sort parameters with `pageInfo.nextCursor` on structured pages.
